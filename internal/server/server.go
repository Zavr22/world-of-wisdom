...
type Cache interface {
	AddCash(int, int64) error
	GetCash(int) (bool, error)
	DeleteCash(int)
}
...
func ProcessRequest(ctx context.Context, msgStr string, clientInfo string) (*protocol.Message, error) {
	...
	case protocol.RequestChallenge:
		...
		err := cache.AddCash(randValue, conf.HashcashDuration)
		if err != nil {
	...
	case protocol.RequestResource:
		...
		exists, err := cache.GetCash(randValue)
		if err != nil {
		...
		_, err = hashcash.CalculateHashcash(maxIter)
		if err != nil {
